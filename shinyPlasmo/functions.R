...
plot_strain_abundances <- function(df, gid) {
  ...
  df %>%
    ...
    theme(axis.title.x = element_text(size=14),
          ...
          plot.title = element_text(vjust=0.5, hjust=0.5, size=16),
...
plot_strain_profiles <- function(df, gid) {
  ...
  df %>%
    ...
    dplyr::mutate(scaled = (log2(TPM+1)-mean(log2(TPM+1)))/sd(log2(TPM+1))) %>%
    ggplot2::ggplot(aes(x = tp, y = scaled, color = toupper(strain))) +
    ylab("Standardized TPM") +
    xlab("") +
    ggplot2::ggtitle(paste(gid)) +
